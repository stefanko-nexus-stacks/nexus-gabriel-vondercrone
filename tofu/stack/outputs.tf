...
output "ssh_command" {
  ...
  value       = "cloudflared access ssh --hostname ssh-gabriel-vondercrone.nona.company"
}
...
output "ssh_config" {
  ...
  value       = <<-EOT
    Host nexus
      HostName ssh-gabriel-vondercrone.nona.company
      User root
...
output "service_urls" {
  ...
  value = {
    ...
    key => "https://${service.subdomain}-gabriel-vondercrone.nona.company"
  }
}
...
output "secrets" {
  ...
  value = {
    ...
    redpanda_schema_registry_public_url = "http://redpanda-schema-registry-gabriel-vondercrone.nona.company:18081"
