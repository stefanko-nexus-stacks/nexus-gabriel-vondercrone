...
SSH_HOST="ssh-gabriel-vondercrone.nona.company"
...
KESTRA_URL=https://kestra-gabriel-vondercrone.nona.company
EOF
...
CB_SERVER_URL=https://cloudbeaver-gabriel-vondercrone.nona.company
...
REDIRECT_URL=https://hoppscotch-gabriel-vondercrone.nona.company
WHITELISTED_ORIGINS=https://hoppscotch-gabriel-vondercrone.nona.company
VITE_BASE_URL=https://hoppscotch-gabriel-vondercrone.nona.company
VITE_SHORTCODE_BASE_URL=https://hoppscotch-gabriel-vondercrone.nona.company
VITE_ADMIN_URL=https://hoppscotch-gabriel-vondercrone.nona.company/admin
VITE_BACKEND_GQL_URL=https://hoppscotch-gabriel-vondercrone.nona.company/backend/graphql
VITE_BACKEND_WS_URL=wss://hoppscotch-gabriel-vondercrone.nona.company/backend/graphql
VITE_BACKEND_API_URL=https://hoppscotch-gabriel-vondercrone.nona.company/backend/v1
...
PREFECT_UI_API_URL=https://prefect-gabriel-vondercrone.nona.company/api
EOF
...
LAKEFS_GATEWAYS_S3_DOMAIN_NAME=s3.lakefs-gabriel-vondercrone.nona.company
...
APPSMITH_CUSTOM_DOMAIN=https://appsmith-gabriel-vondercrone.nona.company
EOF
...
NC_PUBLIC_URL=https://nocodb-gabriel-vondercrone.nona.company
...
EOF
    ...
    if [ -n "$INFISICAL_TOKEN" ] && [ -n "$PROJECT_ID" ]; then
        ...
        build_folder "redpanda" \
            ...
            "REDPANDA_KAFKA_PUBLIC_URL" "redpanda-kafka-gabriel-vondercrone.nona.company:9092" \
            "REDPANDA_SCHEMA_REGISTRY_PUBLIC_URL" "redpanda-schema-registry-gabriel-vondercrone.nona.company:18081" \
            "REDPANDA_ADMIN_PUBLIC_URL" "redpanda-admin-gabriel-vondercrone.nona.company:9644" \
            "REDPANDA_CONNECT_PUBLIC_URL" "redpanda-connect-api-gabriel-vondercrone.nona.company:4195"
        ...
        build_folder "gitea" \
            ...
            "GITEA_REPO_URL" "https://git-gabriel-vondercrone.nona.company/${GITEA_REPO_OWNER:-$ADMIN_USERNAME}/${REPO_NAME:-nexus-${DOMAIN//./-}-gitea}.git" \
            "GITEA_DB_PASSWORD" "$GITEA_DB_PASS"
    ...
    cron: \"*/15 * * * *\"'" >/dev/null 2>&1 || true
            ...
            if echo "$ENABLED_SERVICES" | grep -qw "woodpecker" && [ -n "$WOODPECKER_AGENT_SECRET" ]; then
                ...
                OAUTH_RESULT=$(ssh nexus "curl -s -X POST 'http://localhost:3200/api/v1/user/applications/oauth2' \
                    ...
                    -d '{
                        \"name\": \"Woodpecker CI\",
                        \"redirect_uris\": [\"https://woodpecker-gabriel-vondercrone.nona.company/authorize\"],
...
if echo "$ENABLED_SERVICES" | grep -qw "wikijs" && [ -n "$WIKIJS_ADMIN_PASS" ]; then
    (
        ...
        SETUP_PAYLOAD=$(jq -n \
            ...
            --arg url "https://wiki-gabriel-vondercrone.nona.company" \
